...
if [ "$commit_hash" = "$docker_hash" ]; then
  ...
  echo "The contract in commit hash does not match with hash of contract build in docker. You must call \`make build-reproducible\` command before submitting a PR." >&2
  exit 1
fi
