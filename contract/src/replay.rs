...
const DEFAULT_STATE: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../test_data/account_full_state_190375496.json");
...
#[derive(Debug)]
enum Action {
    /// Oracle records a step-score at its own timestamp.
    RecordScore(Score),
...
impl Action {
    /// Tie-break rank for events sharing a millisecond: scores and deposits
    /// land before a claim so the claim sees the up-to-date state.
    fn rank(&self) -> u8 {
        match self {
            Action::RecordScore(_) => 0,
            Action::Deposit { .. } | Action::SetIncreasedScoreCap(_) => 1,
            Action::Claim => 2,
        }
    }
}
...
fn parse_interactions(path: &str) -> Vec<Event> {
    ...
    for line in raw.lines().skip(1).filter(|l| !l.trim().is_empty()) {
        ...
        let action = match kind {
            ...
            "subscription_activation" if method.contains("increased_score_cap=true") => {
                Action::SetIncreasedScoreCap(true)
            }
            "subscription_activation" if method.contains("increased_score_cap=false") => {
                Action::SetIncreasedScoreCap(false)
            }
            _ => continue,
        };

        events.push(Event { ts, action });
    }

    events
}

fn parse_steps(path: &str) -> Vec<Event> {
    ...
    raw.lines()
        ...
        .map(|line| {
            ...
            Event {
                ts: ts.trim().parse().expect("steps timestamp"),
                action: Action::RecordScore(Score::try_from(steps).unwrap_or(Score::MAX)),
            }
        })
        .collect()
}
...
fn parse_sweat_to_yocto(s: &str) -> u128 {
    ...
    let int: u128 = if int_part.is_empty() { 0 } else { int_part.parse().unwrap() };
...
fn replay_account_history() {
    ...
    let mut events = parse_interactions(&path_from_env("JAR_REPLAY_INTERACTIONS", DEFAULT_INTERACTIONS));
    events.extend(parse_steps(&path_from_env("JAR_REPLAY_STEPS", DEFAULT_STEPS)));

    // Replay window: (snapshot block time, window_end].
    events.retain(|e| e.ts >= snapshot.block_time_ms && e.ts <= window_end);

    // Strict chronological order; ties resolved by `Action::rank`, then file order.
    events.sort_by_key(|e| (e.ts, e.action.rank()));
    ...
    // Store the snapshot account exactly as it was on-chain.
    store_account_raw(
        account_id.clone(),
        Base64VecU8(to_vec(&AccountVersioned::new(snapshot.account)).unwrap()),
    );

    context.set_block_timestamp_in_ms(snapshot.block_time_ms);

    let mut total_claimed: u128 = 0;
    let mut claims = 0u64;

    for event in events {
        context.set_block_timestamp_in_ms(event.ts);
        ...
        match event.action {
            Action::RecordScore(score) => {
                ...
                context
                    .contract()
                    .record_score(vec![(account_id.clone(), vec![(score, UTC(event.ts))])]);
            }
            ...
            Action::Claim => {
                ...
                let PromiseOrValue::Value(claimed) = context.contract().claim_total(None) else {
                    panic!("claim_total returned a promise");
                };
                let amount = claimed.get_total().0;
                total_claimed += amount;
                claims += 1;
                println!("{} - {}", event.ts, amount);
            }
        }
    }
    ...
    assert!(total_claimed > 0, "expected the account to have claimed something");
}
