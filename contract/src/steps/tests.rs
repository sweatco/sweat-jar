...
fn same_interest_in_step_jar_as_in_const_jar() {
    ...
    let mut ctx = TestBuilder::new()
        ...
        .jar(JAR, ())
        .product(STEPS_PRODUCT, [APY(6), StepsCap(steps_cap)])
        .jar(STEP_JAR, ())
        .build();
    ...
    for day in 0..400 {
        ...
        if day % reset_period_days == 0 {
            ctx.record_steps(1, steps_cap + (0..5_000).fake::<Steps>(), alice());
        }
    }
}
...
fn max_steps_apy() {
    ...
    // This product should get 2% APY if user walks 14_000 steps each day
    ...
    let mut ctx = TestBuilder::new()
        .product(PRODUCT_ZERO_CAP, [APY(0), StepsCap(0)])
        .jar(JAR_ZERO_CAP, ())
        .product(PRODUCT_7K_STEPS, [APY(0), NoStepsCap])
        .jar(JAR_7K, ())
