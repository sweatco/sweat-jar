...
async fn test_stage_change() -> Result<()> {
    ...
    ft_contract.new(".u.sweat.testnet".to_string().into()).call().await?;
    ...
    ft_contract
        .storage_deposit(jar_account.to_near().into(), None)
        .call()
        .await?;
    ft_contract
        .tge_mint(&jar_account.to_near(), U128(100_000_000))
        .call()
        .await?;
    ...
    ft_contract.tge_mint(&bob.to_near(), 1_000_000.into()).call().await?;
    ...
    let products = old_jar_contract.get_products().with_user(&ft_account).call().await?;
    assert_eq!(products.len(), 1);

    let bob_jars = old_jar_contract.get_jars_for_account(bob.to_near()).call().await?;
    ...
    assert_eq!(ft_contract.ft_balance_of(bob.to_near()).call().await?.0, 900_000);

    dbg!(ft_contract.ft_balance_of(bob.to_near()).call().await?);
    ...
    let products_new = new_jar_contract.get_products().with_user(&ft_account).call().await?;
    assert_eq!(products, products_new);

    let bob_jars_new = new_jar_contract.get_jars_for_account(bob.to_near()).call().await?;
    ...
    let products = new_jar_contract.get_products().with_user(&ft_account).call().await?;
    ...
    let bob_jars = new_jar_contract.get_jars_for_account(bob.to_near()).call().await?;

    assert_eq!(bob_jars.len(), 2);

    assert_eq!(staked.0, 100_000);

    assert_eq!(ft_contract.ft_balance_of(bob.to_near()).call().await?.0, 800_000);
