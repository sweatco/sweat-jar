...
#[tokio::test]
async fn happy_flow_test() -> anyhow::Result<()> {
    ...
    context
        .ft_contract
        .mint_for_user(manager, 3_000_000)
        .await?;
    context
        .ft_contract
        .mint_for_user(alice, 100_000_000)
        .await?;
    context
        .ft_contract
        .mint_for_user(bob, 100_000_000_000)
        .await?;
    ...
    assert_eq!(
        "2000000",
        alice_first_jar.get("principal").unwrap().as_str().unwrap()
    );

    let alice_second_jar = alice_jars.get(1).unwrap();
    assert_eq!(
        "1",
        alice_second_jar.get("index").unwrap().as_str().unwrap()
    );
    assert_eq!(
        "700000",
        alice_second_jar.get("principal").unwrap().as_str().unwrap()
    );
