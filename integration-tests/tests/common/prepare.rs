...
use sweat_jar_model::jar::JarView;

use super::{ft, jar, product::RegisterProductCommand};

const INITIAL_USER_BALANCE: NearToken = NearToken::from_near(10);
const ALICE_BOB_MANAGER_INITIAL_BALANCE: u128 = 100_000_000;
const JAR_INITIAL_BALANCE: u128 = 100_000_000 * 10u128.pow(18);

const SWEAT_JAR_WASM_ENV: &str = "SWEAT_JAR_WASM";
const SWEAT_JAR_V2_WASM_ENV: &str = "SWEAT_JAR_V2_WASM";
...
pub async fn prepare_contract(products: impl IntoIterator<Item = RegisterProductCommand>) -> Result<Context> {
    ...
    let jar_v2 = deploy(&worker, sweat_jar_v2_wasm_path(), "sweat_jar_v2", SWEAT_JAR_V2_WASM_ENV).await?;
    ...
    ft::tge_mint(&ft, jar.id(), JAR_INITIAL_BALANCE).await?;
    ft::tge_mint(&ft, alice.id(), ALICE_BOB_MANAGER_INITIAL_BALANCE).await?;
    ft::tge_mint(&ft, bob.id(), ALICE_BOB_MANAGER_INITIAL_BALANCE).await?;
    ft::tge_mint(&ft, manager.id(), ALICE_BOB_MANAGER_INITIAL_BALANCE).await?;

    for product in products {
        jar::register_product(&jar, &manager, product.json()).await?;
    }
...
impl Context {
    ...
    pub async fn last_jar_for(&self, account: &Account) -> Result<JarView> {
        Ok(jar::get_jars_for_account(&self.jar, account.id())
            .await?
            .into_iter()
            .last()
            .unwrap())
    ...
    /// `bulk_create_jars` via the manager, minting and transferring enough
    /// SWEAT first so the created jars are token-backed.
...
pub fn init_tracing() {
    let filter = tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        tracing_subscriber::EnvFilter::new("info,workspaces=warn,sandbox=warn,near_workspaces=warn")
    });
...
/// delta, not a real-time duration. This ratio is an empirical calibration so
/// `fast_forward_minutes` advances on-chain time by roughly the requested number
/// of minutes — see `tests/fast_forward.rs`.
...
fn sweat_jar_v2_wasm_path() -> PathBuf {
    wasm_path(SWEAT_JAR_V2_WASM_ENV, repo_path("res", "sweat_jar_v2.wasm"))
}
