...
use crate::{parse, timeline::load_user};
...
fn zero_calc_row(account_id: i64, near_account_id: String, actual: u128, status: String) -> Result<ReconRow> {
    let delta: i128 = 0i128 - i128::try_from(actual).context("actual_total_claim exceeds i128")?;
    Ok(ReconRow {
        account_id,
        near_account_id,
...
pub fn reconcile_user(
    ...
    let (raw_account, no_baseline) = match baseline_raw {
        Err(_) => {
            return zero_calc_row(
                account_id,
                slice.near_account_id,
                actual,
                "error:snapshot panic".to_string(),
            );
        }
        Ok(Err(e)) => {
            return zero_calc_row(
    ...
    let account_id_str: near_sdk::AccountId = slice
        .near_account_id
        .parse()
        .context("near_account_id not a valid AccountId")?;
