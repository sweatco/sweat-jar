...
pub fn explain(opts: &ExplainOpts) -> Result<()> {
    ...
    let outcome = engine::run_timeline(
        engine::Baseline { account_id, raw_account },
        &products,
        parse::H_MS,
        timeline,
    );

    // On-chain claim total per timestamp (jar_events has one row per product).
    let mut onchain: BTreeMap<u64, u128> = BTreeMap::new();
    let mut stmt = conn.prepare(
        "SELECT ts_ms, amount FROM jar_events WHERE account_id = ?1 AND event_type = 'claim'",
    )?;
    let rows = stmt.query_map([opts.account], |r| {
        Ok((r.get::<_, i64>(0)? as u64, r.get::<_, String>(1)?))
    })?;
    for row in rows {
        let (ts, amount) = row?;
        *onchain.entry(ts).or_default() += parse::yocto_str_to_u128(&amount)?;
    }
